...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $msg = "
            <div class='alert-overlay'>
                <div class='alert-box'>
                    ...
                    <p>Solo hay <strong>{$producto['cantidad']}</strong> unidades disponibles.</p>
                    <p>Intentaste vender <strong>{$qty}</strong> unidades.</p>
                    <a href='nueva.php' class='btn btn-danger'>Cerrar</a>
                </div>
            </div>";
  ...
  // Solo crear la venta si no hubo error
  if (!$errorStock) {
    ...
    for ($i = 0; $i < count($productos); $i++) {
      ...
      $ins = $pdo->prepare("
                ...
                (venta_id, producto_id, cantidad, precio_unitario, impuesto, subtotal)
                VALUES (?,?,?,?,?,?)
            ");
      ...
      <form method="POST" id="ventaForm">
        ...
        <div id="items">
          <div class="item-row form-row" style="align-items:end;margin-bottom:10px">
            ...
            <div class="form-group" style="max-width:120px"><label>Cantidad</label><input name="cantidad[]"
                type="number" min="1" value="1"></div>
          </div>

        </div>
        <button type="button" class="btn btn-ghost" style="margin-bottom:18px" onclick="addItem()"><i
